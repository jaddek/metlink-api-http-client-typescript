...
export class MetlinkHttpClientBuilder {
    public static buildWithAxios(
        ...
        const [host, headers]: [HostInterface, HeadersDictInterface] =
            HeadersDirector.build(token)
        const config = {
            baseURL: host.getUrl(),
            headers: headers,
        }
    ...
    public static buildWithAxiosUsingResponseDataDecorator(
        ...
        return new ResponseDataDecorator(
            new MetlinkHttpClient(new AxiosAdapter(axiosInstance))
        )
    }
}
